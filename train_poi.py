...
from dataloader import MNIST01, STL_monkey
...
def main(args, model_name, trainloader, testloader, poiloader_train=None, poiloader_test=None):
    ...
    if mode == "reg":
        best_model, best_acc = engine.train_reg(args, net, criterion, optimizer, trainloader, testloader, n_epochs, poiloader_train, poiloader_test)
    elif mode == "vib":
        best_model, best_acc = engine.train_vib(args, net, criterion, optimizer, trainloader, testloader, n_epochs, poiloader_train, poiloader_test)
	
    torch.save({'state_dict':best_model.state_dict()}, os.path.join(model_path, "{}_poi_{:.2f}.tar").format(model_name, best_acc))
...
if __name__ == '__main__':
    ...
    # get poison loader 
    # STL10 class 7 is monkey, CIFAR10 class 6 is frog (USE monkey for cat), class3 is cat
    root = '/home/chensi/data/'

    poi_trainset = STL_monkey(root=root, split='train')
    poi_testset = STL_monkey(root=root, split='test')

    poiloader_train = torch.utils.data.DataLoader(poi_trainset, batch_size=64, 
            shuffle=True, sampler=None)
    poiloader_test = torch.utils.data.DataLoader(poi_trainset, batch_size=64, 
            shuffle=True, sampler=None)


    main(args, model_name, trainloader, testloader, poiloader_train, poiloader_test)