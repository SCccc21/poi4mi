...
def inversion(G, D, T, E, iden, itr, lr=2e-2, momentum=0.9, lamda=100, iter_times=1500, clip_range=1, improved=False, num_seeds=20, save_img_dir=None):
	...
	for random_seed in range(num_seeds):
		...
		z = Variable(torch.randn(bs, 100), requires_grad=True)
		# optimizer = optim.SGD(params=[z], lr=lr, momentum=momentum, nesterov=True)
		# optimizer = optim.Adam(params=[z], lr=lr)
		optimizer = SGLD(params=[z], lr=lr)

		# scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=4500, verbose=False)
		...
		for i in range(iter_times):
			...
			# scheduler.step()
			with torch.no_grad():
				z.clamp_(-clip_range, clip_range)
		...
		if iden[0] == 38:
			# pickle.dump(z.detach(), open('./final_z_surrogateLargeWeight_seed{}.data'.format(random_seed), 'wb'))
		...
		lll = nn.CrossEntropyLoss(reduction='none')(out, iden)
		print("Iden Loss:", lll)
